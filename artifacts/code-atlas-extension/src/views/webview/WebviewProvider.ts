...
export class WebviewProvider {
  ...
  async open(entryId: string): Promise<void> {
    ...
    const column =
      config.get<string>('openDocPanel') === 'active'
  ...
  private render(panel: vscode.WebviewPanel, entry: ApiEntry): void {
      ...
      related.length > 0
        ...
        <div class="related-grid">
          ${related
            .map(
              (r) =>
                `<button class="related-chip" onclick="navigate('${this.escapeHtml(r.id)}')">
